...
public class ArmeriaHttpClientConfig implements HttpClientConfig {
     ...
     * {@code https}.
     *
     * @param protocolPreference specifies which protocol shall take preference. Acceptable values: {@code H1},
     * {@code HTTP1}, {@code HTTP/1.1} for {@code HTTP/1.1} and {@code H2}, {@code HTTP2}, {@code HTTP/2} for
     * {@code HTTP/2}.
     */
    ...
    /**
     * Sets the client to retry when the remote end-point responds with one of the specified {@code statuses}.
     *
     * Uses a backoff that computes a delay using one of supported functions, such as
     * {@code exponential(long, long, double)}, {@code fibonacci(long, long)}, {@code fixed(long)} and
     * {@code random(long, long)} chaining with {@code withJitter(double, double)} and {@code withMaxAttempts(int)} from
     * the {@code backoff} string that conforms to the following format:
     * <ul>
     ...
     * <li>{@code fibonacci=[initialDelayMillis:maxDelayMillis]} is for {@code fibonacci(long, long)}</li>
     ...
     * <li>{@code jitter=[minJitterRate:maxJitterRate]} is for {@code withJitter(double, double)} (if only one jitter
     * value is specified, it will be used for {@code withJitter(double)}</li>
     * <li>{@code maxAttempts=[maxAttempts]} is for {@code withMaxAttempts(int)}</li>
     * </ul>
     * The order of options does not matter, and the {@code backoff} needs at least one option. If you don't specify the
     * base option exponential backoff will be used. If you only specify a base option, jitter and maxAttempts will be
     * set by default values. For example:
     * <ul>
    ...
    /**
     * Sets the client to log requests and responses to the specified {@code logger}. This method explicitly specifies
     * various log levels. The log level correspond to a value of {@link LogLevel} and must use one of the following
     * values: {@code ["OFF", "TRACE", "DEBUG", "INFO", "WARN", "ERROR"]}
     *
     ...
     * @param failureResponseLevel the log level to use for logging error responses, default {@code "WARN"}
     */
    ...
    /**
     * Sets the client to log requests and responses to the specified {@code logger} using default log levels.
     *
     * @param logger the logger name (of {@link org.slf4j.Logger}) to log requests/responses to
     */
