...
public class OAuth2JsonAccessTokenExtractor implements AccessTokenExtractor {
    ...
    public OAuth2JsonAccessTokenExtractor(String accessTokenKey, String refreshTokenKey,
            String expiresInKey, String expiresAtKey, String tokenTypeKey) {
    ...
    public AccessToken extract(final String response) {
        ...
        final Matcher accessTokenMatcher =
                Pattern.compile("\"" + accessTokenKey + "\"\\s*:\\s*\"(\\S*?)\"").matcher(response);
        final Matcher refreshTokenMatcher =
                Pattern.compile("\"" + refreshTokenKey + "\"\\s*:\\s*\"(\\S*?)\"").matcher(response);
        final Matcher expiresInMatcher =
                Pattern.compile("\"" + expiresInKey + "\"\\s*:\\s*(\\d*?)\\D").matcher(response);
        final Matcher expiresAtMatcher =
                Pattern.compile("\"" + expiresAtKey + "\"\\s*:\\s*(\\d*?)\\D").matcher(response);
        final Matcher tokenTypeMatcher =
                Pattern.compile("\"" + tokenTypeKey + "\"\\s*:\\s*\"(\\S*?)\"").matcher(response);
        ...
        if (accessTokenMatcher.find()) {
            final String accessToken = accessTokenMatcher.group(1);
