...
public class OAuth10aServiceImpl implements OAuthService {
	...
	/**
	 * Constructor with flag of using QueryString
	 * 
	 * @param api
	 *            OAuth1.0a api information
	 * @param config
	 ...
	 *            if is true, oauth parameters will send in querystring, or will send in header
	 */
	public OAuth10aServiceImpl(DefaultApi10a api, OAuthConfig config, Boolean useQueryString) {
		this.api = api;
		this.config = config;
		this.scope = NO_SCOPE;
	...
	private void addOauth(OAuthRequest request) {
		if(useQueryString) {
			addOauthQuerystring(request);
		} else {
	...
	private void addOauthQuerystring(OAuthRequest request) {
